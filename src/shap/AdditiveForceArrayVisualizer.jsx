...
class AdditiveForceArrayVisualizer extends React.Component {
  ...
  componentDidMount() {
    ...
    this.svg.on('click', () =>
      alert(
        'This original index of the sample you clicked is ' +
          this.nearestExpIndex
      )
    );
  ...
  mouseMoved() {
    ...
    let x = d3.pointer(this.svg.node())[0];
  ...
  internalDraw() {
    ...
    if (xsort === 'sample order by similarity') {
      ...
      if (isTimeScale) {
        each(
          explanations,
          (e, i) => (e.xmap = this.parseTime(this.props.ordering_keys[i]))
        );
      } else {
